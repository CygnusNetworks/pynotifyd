...
def validate_contact(contact):
	...
	for number in get_the_item(contact, "number"):
		if not number.startswith("+"):
			raise PyNotifyDConfigurationError("phone number must start with a plus sign")
		if not number[1:].isdigit():
			raise PyNotifyDConfigurationError("non-digits found in phone number")

	for jabber in get_the_item(contact, "jabber"):
		if '@' not in jabber:
			raise PyNotifyDConfigurationError("a jabberid has to contain an @ sign")
...
def read_config(filename):
	...
	for section_list, key, error in configobj.flatten_errors(config, config.validate(validate.Validator())):
		raise PyNotifyDConfigurationError("Failed to validate %s in section %s." % (key, ", ".join(section_list)))

	# check contacts
	for contactname, contact in config["contacts"].items():
		if not isinstance(contact, dict):
			raise PyNotifyDConfigurationError("non-section found in section contacts")
		try:
			validate_contact(contact)
		except PyNotifyDConfigurationError, err:
			raise PyNotifyDConfigurationError("%s in contact %s" % (err.message, contactname))
	...
	for provider in config["general"]["retry"]:
		...
		if provider in config["providers"]:
			continue
		raise PyNotifyDConfigurationError("provider %s not found" % provider)
	return config
